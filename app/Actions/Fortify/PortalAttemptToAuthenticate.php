...
class PortalAttemptToAuthenticate
{
    /**
     * Handle the incoming request for PORTAL login (Tourist only).
     */
    public function __invoke($request): void
    {
        ...
        // Check if user role is Tourist - PORTAL LOGIN IS FOR TOURISTS ONLY
        if (! $user->role || $user->role->name !== 'Tourist') {
            throw ValidationException::withMessages([
                Fortify::username() => [
                    'Only Tourist accounts can login through this portal. Please use the appropriate login page for your role.'
                ],
            ]);
        }
